...
export const orderNewProject = async (orderInfo) => {
    try{
        const response = await instance.post('api/v1/project/create-orderedProject',orderInfo )
...
export const approveOrder = async (orderId, updateOrder) => {
    try {
        const response = await instance.put(`api/v1/project/update-orderStatus/${orderId}`, updateOrder);
...
export const orderSenderRoleUpdate = async (orderSenderEmail, updateBuyerRole) => {
    try {
        const response = await instance.put(`api/v1/user/role-update?email=${orderSenderEmail}`, updateBuyerRole);
        console.log(response)
        return response;
        } catch (error) {
        console.log(error.message);
...
export const projectContributorsUpdate = async (id,addedDevContributors) => {
    try {
        const response = await instance.put(`api/v1/project/update-contributors/${id}`, addedDevContributors);
        console.log(response)
        return response;
        } catch (error) {
        console.log(error.message);
...
export const sendSelectedProjectDevelopers = async (mailInfo) => {
    try {
        const response = await instance.post('api/v1/sendMail/mailToContributor', mailInfo);
        const data = await response;
        console.log(data);
        return data?.data;
        } 
