...
import { approveOrder, orderSenderRoleUpdate } from "../../../apis/orderOperation";
...
const OrderRequestTable = ({ newOrderRequest }) => {
  ...
  const handleApprove = (data) => {
    ...
    approveOrder(orderId,updateOrder)
      .then((response) => {
  ...
  const handleRoleUpdate = (data) => {
    ...
    orderSenderRoleUpdate(orderSenderEmail, updateBuyerRole)
      .then((response2) => {
      ...
      <table className="min-w-full divide-y-2 divide-gray-200 bg-white dark:bg-black text-sm">
        ...
        <tbody className="divide-y divide-gray-200">
          {newOrderRequest?.map((data, i) => (
              <tr key={i} className="hover:bg-slate-50 dark:hover:bg-[#82f8d9] group">
