...
const AllApproveOrdersHeader = ({ orderList }) => {
  const { setFilteredData, filteredData, setHasSearched } =
  ...
  const handleSubmit = (data) => {
    ...
    const filtered = orderList?.filter((item) => {
      return item?.projectName?.toLowerCase().includes(data);
    });
  ...
  return (
    <div className="flex justify-between mx-5">
      <div className="space-y-2">
        <h1 className="text-2xl title font-semibold">Selosia</h1>
        ...
        <div className="search-dropdown">
          {orderList
            ?.filter((item) => {
              ...
              return (
                searchItem &&
                categoryItems?.startsWith(searchItem) &&
                item?.projectName !== searchItem
              );
            })
            ?.map((item, idx) => (
              <div
